...
export class ListFormationsComponent implements OnInit {
  ...
  addFormation(event): void{
    var formation = {
      "titre": event.newData.titre,
      "annee": parseInt(event.newData.annee),
      "duree":  parseInt(event.newData.duree),
      "budget": parseFloat(event.newData.budget),
      "nb_session": parseInt(event.newData.nb_session),
  ...
  onUserRowSelect(event): void{
    ...
    console.log(this.httpFormationService._selectedFormation)
    this.router.navigate(['userPages/sessions'])
  }
